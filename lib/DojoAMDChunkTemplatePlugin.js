...
module.exports = class DojoAMDChunkTemplatePlugin {
	constructor(options) {
		...
		/* Prepare a regexp for matching tolerated duplicated modules */
		if (options.ignoredConflictingModules && options.ignoredConflictingModules.length > 0) {
			this.ignoredConflictingModulesPattern = new RegExp("^(?:" + options.ignoredConflictingModules.map(s => {
	...
	renderAbsMids(source, chunk) {
		...
		const renderAbsMid = function(absMid, mod) {
			if (!renderedAbsMids.hasOwnProperty(absMid)) {
			    ...
			    if (!this.ignoredConflictingModulesPattern || (!this.ignoredConflictingModulesPattern.test(renderedAbsMids[absMid].request) && !this.ignoredConflictingModulesPattern.test(mod.request))) {
